...
def tree():
    ...
    # prog = 'function(int) foo(int x, int y) {x=x+1;y=x+1;z=x+y; return z;} x=foo(1,2); y = foo(2,3); z= 5;'
    ...
    prog = 'function (float) foo(int n) {epsilon = 0.001; x = 1; r = sqrt(n); while (abs(x - r) > epsilon) x = 0.5 * (x + n/x); return x;} y = foo(25);'
